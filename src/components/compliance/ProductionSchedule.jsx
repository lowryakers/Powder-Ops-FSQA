...
import { ChevronLeft, ChevronRight, Calendar, Share2, Plus, X, ChevronDown, Check } from 'lucide-react';
...
function CellModal({ cell, weekStart, dayIndex, room, roomType, userName, onClose, onSaved }) {
...
export default function ProductionSchedule({ user }) {
  ...
  const [editCell, setEditCell] = useState(null); // { dayIndex, room, roomType }
    ...
    for (let d = 0; d < 5; d++) {
      ...
      for (const section of ROOM_SECTIONS) {
        for (const room of section.rooms) {
          const a = assignmentMap[`${d}-${room}`];
          if (a && (a.team || a.mo_number || a.product_name)) {
