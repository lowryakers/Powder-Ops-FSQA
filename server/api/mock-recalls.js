...
router.post('/', (req, res) => {
  const db = getDb();
  const { date_initiated, product_name, lot_number, reason, initiated_by, scope, quantity_produced, quantity_distributed, quantity_recovered, distribution_list, time_to_notify_minutes, time_to_complete_minutes, accounts_contacted, accounts_responded, effectiveness_pct, result, corrective_actions, notes, _actor } = req.body;
  ...
  db.prepare(`INSERT INTO mock_recalls (id, recall_number, date_initiated, product_name, lot_number, reason, initiated_by, scope, quantity_produced, quantity_distributed, quantity_recovered, distribution_list, time_to_notify_minutes, time_to_complete_minutes, accounts_contacted, accounts_responded, effectiveness_pct, result, corrective_actions, notes)
    ...
    product_name, lot_number, reason, initiated_by || (_actor || 'system'),
  ...
  logAudit(_actor || 'system', 'mock_recall_created', 'mock_recall', id, { recall_number: nextNum, product_name });
...
router.put('/:id', (req, res) => {
  ...
  const { date_initiated, product_name, lot_number, reason, initiated_by, scope, quantity_produced, quantity_distributed, quantity_recovered, distribution_list, time_to_notify_minutes, time_to_complete_minutes, accounts_contacted, accounts_responded, effectiveness_pct, result, corrective_actions, notes, completed_at, _actor } = req.body;
  ...
  logAudit(_actor || 'system', 'mock_recall_updated', 'mock_recall', req.params.id, { result: newResult });
